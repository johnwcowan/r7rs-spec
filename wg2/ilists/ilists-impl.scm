...
;;; - It relies on similar properties from the other ilist-lib procs it calls.
...
(define (ilset<= = . lists)
  ...
  (or (not (ipair? lists)) ; 0-ary case
	...
	(or (not (ipair? rest))
	    (let ((s2 (icar rest))  (rest (icdr rest)))
...
(define (ilset-adjoin = lis . elts)
  (check-arg procedure? = ilset-adjoin)
  (ifold (lambda (elt ans) (if (imember elt ans =) ans (ipair elt ans)))
	lis elts))


(define (ilset-union = . lists)
  (check-arg procedure? = ilset-union)
  (ireduce (lambda (lis ans)		; Compute ANS + LIS.
	    (cond ((null? lis) ans)	; Don't copy iany lists
		  ((null? ans) lis) 	; if we don't have to.
		  ((eq? lis ans) ans)
		  (else
		   (ifold (lambda (elt ans) (if (iany (lambda (x) (= x elt)) ans)
					       ans
					       (ipair elt ans)))
			 ans lis))))
	  '() lists))

(define (ilset-intersection = lis1 . lists)
  (check-arg procedure? = ilset-intersection)
  (let ((lists (idelete lis1 lists eq?))) ; Throw out iany LIS1 vals.
    (cond ((iany null-ilist? lists) '())		; Short cut
	  ((null? lists)          lis1)		; Short cut
	  (else (ifilter (lambda (x)
			  (ievery (lambda (lis) (imember x lis =)) lists))
			lis1)))))

(define (ilset-difference = lis1 . lists)
  (check-arg procedure? = ilset-difference)
  (let ((lists (ifilter ipair? lists)))	; Throw out empty lists.
    (cond ((null? lists)     lis1)	; Short cut
	  ((imemq lis1 lists) '())	; Short cut
	  (else (ifilter (lambda (x)
			  (ievery (lambda (lis) (not (imember x lis =)))
				 lists))
			lis1)))))

(define (ilset-xor = . lists)
  (check-arg procedure? = ilset-xor)
  (ireduce (lambda (b a)			; Compute A xor B:
	    ;; Note that this code relies on the constant-time
	    ;; short-cuts provided by ILSET-DIFF+INTERSECTION,
	    ;; ILSET-DIFFERENCE & IAPPEND to provide constant-time short
	    ;; cuts for the cases A = (), B = (), and A eq? B. It takes
	    ;; a careful case analysis to see it, but it's carefully
	    ;; built in.

	    ;; Compute a-b and a^b, then compute b-(a^b) and
	    ;; ipair it onto the front of a-b.
	    (receive (a-b a-int-b)   (ilset-diff+intersection = a b)
	      (cond ((null? a-b)     (ilset-difference = b a))
		    ((null? a-int-b) (iappend b a))
		    (else (ifold (lambda (xb ans)
				  (if (imember xb a-int-b =) ans (ipair xb ans)))
				a-b
				b)))))
	  '() lists))


(define (ilset-diff+intersection = lis1 . lists)
  (check-arg procedure? = ilset-diff+intersection)
  (cond ((ievery null-ilist? lists) (values lis1 '()))	; Short cut
	((imemq lis1 lists)        (values '() lis1))	; Short cut
	(else (ipartition (lambda (elt)
			   (not (iany (lambda (lis) (imember elt lis =))
				     lists)))
			 lis1))))
