...
\defining{REPL} (Read-Eval-Print-Loop), where Scheme expressions are
...
\vest In particular, implementations that provide more than one representation
...
(as by {\cf sqrt}), then
the most precise representation available must be used; but if the result
is represented in some other way then the representation must have at least as
much precision as the most precise representation available.
...
pairs, the empty list, symbols, numbers, strings, vectors,
...
definitions from the file as though wrapped in {\cf begin}.
