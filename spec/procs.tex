...
After printing, the second example then behaves in an unspecified way.
\end{entry}
...
behave exactly as if the current input/output port had been bound
...
\begin{scheme}
(get-features) \ev
  (r7rs ratios complex threads full-unicode
    modules string-streams bidir-ports
    dynamic-loading linux little-endian)
\end{scheme}
\end{entry}
