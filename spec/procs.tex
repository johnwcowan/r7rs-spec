...
\begin{entry}{%
\proto{parameterize}{ ((param value) \dotsfoo) \hyperi{expr} \hyperii{expr} \dotsfoo}{library syntax}}
...
in an unspecified order.  The value of the \var{param} expressions
must be parameter objects.  The \hyper{expr}s are evaluated in order
in a dynamic extent during which calls to the \var{param} parameter
objects return the result of the corresponding
\texttt{(\var{converter} \var{value})}.  The result(s) of the last
\hyper{expr} is(are) returned as the result(s) of the entire
