...
\begin{entry}{%
\proto{finite?}{ \vr{z}}{procedure}
\proto{infinite?}{ \vr{z}}{procedure}
\proto{nan?}{ \vr{z}}{procedure}}
...
\vrii{n} should be non-zero.  All fifteen procedures return integers.
The procedures compute a quotient \vr{n_q} and remainder \vr{n_r} such
...
\begin{scheme}
(\hyper{operator}/)             \ev ({\cf values} \vr{n_q} \vr{n_r})
(\hyper{operator}-quotient)     \ev \vr{n_q}
(\hyper{operator}-remainder)    \ev \vr{n_r}
\end{scheme}
...
\begin{entry}{%
\proto{list-copy}{ list}{library procedure}}
...
\begin{entry}{%
\proto{vector->string}{ string}{library procedure}
\proto{string->vector}{ vector}{library procedure}}
...
defines \hyper{kons} to be a constructor, \hyper{kar} and \hyper{kdr}
to be accessors, \hyper{set-kar!} to be a modifier, and \hyper{pare?}
to be a predicate for \hyper{<pare>}s.
...
the blob is created.  The {\em valid indexes}\index{valid indexes} of
a blob are the exact non-negative integers less than the length of the
blob, starting at index zero as with vectors.

\begin{entry}{%
\proto{blob?}{ obj}{procedure}}

Returns \schtrue{} iff \var{obj} is a blob.
\end{entry}

\begin{entry}{%
\proto{make-blob}{ k}{procedure}}

Returns a newly allocated blob of \var{k} bytes.  The initial contents
of each element is unspecified.
\end{entry}

\begin{entry}{%
\proto{blob-length}{ blob}{procedure}}

Returns the length of \var{blob} in bytes as an exact integer.
\end{entry}

\begin{entry}{%
\proto{blob-u8-ref}{ blob k}{procedure}}

Returns the \var{k}th byte of \var{blob} as an exact integer in the
range $[0..255]$.
\end{entry}

\begin{entry}{%
\proto{blob-u8-set!}{ blob k u8}{procedure}}

Stores \var{u8} as the \var{k}th byte of \var{blob}.  \var{u8} must be
an exact integer in the range $[0..255]$.  The value returned by
{\cf blob-u8-set!} is unspecified.
\end{entry}

\begin{entry}{%
\proto{copy-blob}{ blob}{procedure}}
...
\begin{entry}{%
\proto{copy-blob!}{ from to}{procedure}}

Copy the bytes of blob \var{from} to blob \var{to}, which must not be
shorter.  The value returned by {\cf copy-blob!} is unspecified.
\end{entry}

\begin{entry}{%
\proto{partial-blob}{ blob start end}{procedure}}

Returns a newly allocated blob containing the same bytes in \var{blob}
starting at \var{start} (inclusive) and ending at \var{end}
(exclusive).
\end{entry}

\begin{entry}{%
\proto{copy-partial-blob!}{ from start end to at}{procedure}}

Copy the bytes of blob \var{from} starting at \var{start} and ending
at \var{end} to blob \var{to}, starting at \var{at}.  The inequality
\texttt{({\cf >=} ({\cf -} ({\cf blob-length} \var{to}) \var{at}) ({\cf -} \var{end} \var{start}))}
...
\begin{entry}{%
\proto{string-map}{ proc \vari{string} \varii{string} \dotsfoo}{library procedure}}
...
\begin{entry}{%
\proto{vector-map}{ proc \vari{vector} \varii{vector} \dotsfoo}{library procedure}}
...
\begin{entry}{%
\proto{string-for-each}{ proc \vari{string} \varii{string} \dotsfoo}{library procedure}}
...
(let ((v (make-vector 5)))
  (string-for-each
   (lambda (c) (vector-set! v c (char->integer c)))
   "abcde")
...
\begin{entry}{%
\proto{vector-for-each}{ proc \vari{vector} \varii{vector} \dotsfoo}{library procedure}}
...
returned by the call \texttt{(\var{converter} \var{init})}. If the
conversion procedure \var{converter} is not specified the identity
...
\begin{entry}{%
\pproto{(parameterize ((param value) \dotsfoo)}{library syntax}
{\tt\obeyspaces%
\hspace*{1em}\hyperi{expr} \hyperii{expr} \dotsfoo)}}
...
\var{param} and \texttt{(\var{converter} \var{value})} are evaluated
in an unspecified order.  The \hyper{expr}s are evaluated in order in
a dynamic extent during which calls to the \var{param} parameter
...
\todo{What to say about threads?  The dynamic extent of a parameterize
  form should not be visible to existing threads.  Child threads
  created in that extent are handled differently by different
  implementations.}
\end{entry}
...
Raises a \defining{continuable exception} by invoking the current
...
information provided by \var{message} and any \var{obj}s.
\end{entry}
...
output port), respectively.  The initial bindings for each of these
are bound to system defined binary ports.

\todo{Shinn: Are these parameters?}
...
\proto{include}{ filename}{optional syntax}}
...
\begin{entry}{%
\proto{file-exists?}{ filename}{optional procedure}}
...
\begin{entry}{%
\proto{delete-file}{ filename}{optional procedure}}
...
\begin{entry}{%
\proto{get-environment-variable}{ name}{optional procedure}}
...
\begin{entry}{%
\proto{get-environment-variables}{}{optional procedure}}
...
\begin{entry}{%
\proto{current-second}{}{optional procedure}}
...
\begin{entry}{%
\proto{current-jiffy}{}{optional procedure}}
...
\begin{entry}{%
\proto{jiffies-per-second}{}{optional procedure}}
