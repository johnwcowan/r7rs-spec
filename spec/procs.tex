...
but only allow characters U+0001 to U+00FF (the Latin-1 repertoire) in strings.
...
(see \ref{vectortostring}),
or in UTF-8 encoded form within a bytevector passed to
{\cf utf8\coerce{}string} (see \ref{utf8tostring}).
It is also an error for a procedure passed to {\cf string-map}
(see \ref{stringmap}) to return a forbidden character,
or for {\cf read-string} (see \ref{readstring})
...
\domain{It is an error if \var{char} is forbidden in strings.}
...
\domain{It is an error if any of the \var{char}s are forbidden in strings.}
...
\domain{It is an error if \vr{k} is not a valid index of \var{string}.
It is also an error if \var{char} is forbidden in strings.}
...
\begin{note}
This procedure appears in R6RS, but places the source before the destination,
...
It is an error if any of the characters returned by \var{proc}
is forbidden in strings.
...
It is an error if any of the characters read are forbidden in strings.
