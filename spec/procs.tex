...
true.  On empty vectors and empty strings, {\cf eq?} may also behave
...
\mainschindex{cdar}\mainschindex{cddr}%
\mainschindex{caaar}\mainschindex{caadr}\mainschindex{cadar}\mainschindex{caddr}%
...
If the last pair of \var{list} has a cdr which is not the empty
list, the last pair of the result does, too.  
An argument which is not a list is returned unchanged.
...
\begin{entry}{%
\proto{string-ci=?}{ \vri{string} \vrii{string} \vriii{string} \dotsfoo}{string library procedure}}
...
\begin{entry}{%
\proto{string-ni=?}{ \vri{string} \vrii{string} \vriii{string} \dotsfoo}{procedure}}
...
NFD as specified by Unicode UAX\#15~\cite{uax15}.  Implementations which only
...
Note that this is both the external representation of a vector and an
expression evaluating to a vector.  
...
and return a single value.  If more
...
and return a single character.  If more
...
{\cf let-values} and {\cf let*-values} expressions),
...
\var{thunk} does not accept zero arguments.}  The {\cf
...
case they should return an environment containing bindings corresponding to the corresponding version of the report.
If \var{version}
...
{\cf open-output-string}.}  Returns a string consisting of the
