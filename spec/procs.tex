...
\proto{infinite?}{ \vr{z}}{inexact module procedure}
...
\begin{scheme}
(\hyper{operator}/)             \ev \vr{n_q} \vr{n_r}
...
\begin{entry}{%
\proto{blob-copy}{ blob}{procedure}}
...
\begin{entry}{%
\proto{blob-copy!}{ from to}{procedure}}
...
\begin{entry}{%
\proto{partial-blob-copy!}{ from start end to at}{procedure}}
...
must be true.  The value returned by {\cf partial-blob-copy!} is unspecified.
\end{entry}
...
Whether the input and output port types are disjoint is implementation-dependent.
...
A binary port supports reading or writing of individual octets from
or to a backing store containing octets using {\cf read-u8} and {\cf
write-u8}, below. Every binary port is also automatically a character
port, by treating the sequence of octets as an implementation-determined
character set encoding. Conversely, not all character ports are binary
ports - for example, the {\em string ports} discussed below.
...
be a string or a {\em file-spec}, which is a list of alternating
\hyper{keys} and \hyper{values} where every \hyper{key} is a symbol.
Specifying a string is equivalent to the file-spec \texttt{({\cf
...
\begin{entry}{%
\proto{call-with-input-file}{ file-spec proc}{file module procedure}
\proto{call-with-output-file}{ file-spec proc}{file module procedure}}
...
\begin{entry}{%
\proto{with-input-from-file}{ file-spec thunk}{file module procedure}
\proto{with-output-to-file}{ file-spec thunk}{file module procedure}}
...
\begin{entry}{%
\proto{open-input-file}{ file-spec}{file module procedure}}
...
\begin{entry}{%
\proto{open-output-file}{ file-spec}{file module procedure}}
...
\begin{entry}{%
\proto{write-u8}{ octet}{io module procedure}
\rproto{write-u8}{ octet port}{io module procedure}}
