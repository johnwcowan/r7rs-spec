...
\begin{entry}{%
\proto{finite?}{ \vr{x}}{procedure}
\proto{nan?}{ \vr{x}}{procedure}}
...
division operators,
there are three procedures defined as
follows:
...
Returns a newly allocated blob containing the bytes in \var{blob}
...
Copy the bytes of blob \var{from} between \var{start} and \var{end}
to blob \var{to}, starting at \var{at}.  The order in which bytes are
...
(vector-map + '\#(1 2 3) '\#(4 5 6))         \lev  \#(5 7 9)

(let ((count 0))
  (vector-map (lambda (ignored)
         (set! count (+ count 1))
         count)
       '\#(a b)))                 \lev  \#(1 2) \var{or} \#(2 1)
\end{scheme}
...
the information provided by \var{message} (which should be a string)
and any \var{obj}s.
\end{entry}
...
\begin{entry}{%
\proto{interaction-environment}{}{procedure}}
...
\begin{entry}{%
\proto{call-with-input-file}{ file-spec proc}{procedure}
\proto{call-with-output-file}{ file-spec proc}{procedure}}
...
output port), respectively.  All three ports are parameters.

\end{entry}


\begin{entry}{%
\proto{with-input-from-file}{ file-spec thunk}{procedure}
\proto{with-output-to-file}{ file-spec thunk}{procedure}}
...
Takes a \var{file-spec} for an existing file and returns a binary input
...
Takes a \var{file-spec} naming an output file to be created and returns
...
incapable of delivering or accepting characters or bytes.  \todo{But maybe a no-op
...
\rproto{write}{ obj port}{procedure}}
...
\rproto{display}{ obj port}{procedure}}
...
\begin{entry}{%
\proto{load}{ file-spec}{procedure}
...
\begin{entry}{%
\proto{get-environment-variable}{ name}{procedure}}
...
\begin{entry}{%
\proto{get-environment-variables}{}{procedure}}
...
\begin{entry}{%
\proto{current-second}{}{procedure}}
...
\begin{entry}{%
\proto{jiffies-per-second}{}{procedure}}
