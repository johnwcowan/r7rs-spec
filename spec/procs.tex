...
\item \vari{obj} and \varii{obj} are both numbers, 
have the same exactness,
are numerically equal (see {\cf =}, section~\ref{numbersection}),
and it is not the case that one is {\cf 0.0} and the other is {\cf -0.0}.
...
vectors, strings, bytevectors, and records, applying {\cf eqv?} on booleans,
...
\rproto{vector-copy}{ vector start end fill}{procedure}}
...
If \var{end} is greater than the length of \var{vector}, the
\var{fill} argument is used to fill the additional elements of the result.
If \var{fill} is omitted, an implementation-defined value is used.
...
Runs all outstanding dynamic-wind \var{after} procedures, terminates the running program and communicates an exit value to the
...
exited normally.  If \var{obj} is supplied, the exit procedure
should translate it into an appropriate exit value for the
operating system. If \var{obj} is \schfalse{}, the exit is assumed to be
abnormal.
