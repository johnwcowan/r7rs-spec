...
A program may use a global variable definition to bind any variable.  It may
...
the IEEE 754-2008 standard, and have the same radix,
...
\item \vari{obj} and \varii{obj} are both inexact real numbers,
are not implemented in the style of
IEEE 754-2008, and are numerically equal (see {\cf =}),
...
\item \vari{obj} and \varii{obj} are both inexact real numbers
implemented in the style of
the IEEE 754-2008 standard, and have different radices,
...
\item \vari{obj} and \varii{obj} are both inexact real numbers
not implemented in the style of
IEEE 754-2008 for which {\cf =} returns \schfalse{},
provided that they are not both representations of NaNs.
...
Negative zero is an inexact real value written {\cf -0.0} and is normally distinct
...
inexact numbers are represented by 64-bit IEEE floating point numbers.
...
is false.  Converting inexact values to exact equivalents will avoid
...
such circumstances.  If omitted \var{start} defaults to zero and \var{end}
defaults to {\cf (min (+ start (- (string-length to) at)) (string-length from))}.
...
allocating storage by making sure to copy in the correct direction in
such circumstances.  If omitted \var{start} defaults to zero and \var{end}
defaults to {\cf (min (+ start (- (vector-length to) at)) (vector-length from))}.
...
allocating storage by making sure to copy in the correct direction in
such circumstances.  If omitted \var{start} defaults to zero and \var{end}
defaults to {\cf (min (+ start (- (bytevector-length to) at)) (bytevector-length from))}.
...
However, {\cf display} must not loop forever on circular lists or
self-referencing vectors.  Thus if the
