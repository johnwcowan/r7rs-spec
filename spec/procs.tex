...
A program may use a top-level variable definition to bind any variable.  It may
...
\item \vari{obj} and \varii{obj} are both inexact real numbers
...
\item \vari{obj} and \varii{obj} are both complex numbers whose
...
\item \vari{obj} and \varii{obj} are both exact numbers for which
...
In particular, a \defining{exact complex number} must have an exact real part
...
\begin{scheme}
-                   *                  +
abs                 ceiling            denominator
exact-integer-sqrt  expt               floor
floor/              floor-quotient     floor-remainder
gcd                 lcm                max
min                 modulo             numerator
quotient            rationalize        remainder
round               square             truncate
truncate/           truncate-quotient  truncate-remainder
\end{scheme}
exact-integer-sqrt  square
...
\proto{log}{ \vri{z} \vrii{z}}{inexact library procedure}
...
if a single argument is given, or the logarithm of \vri{z} to the base
\vrii{z} if two arguments are given.
...
The value of $0^z$ is $1$ is {\cf (zero? z)}, $0$ if {\cf (real-part z)}
...
returns \schfalse{}.
An error is never signalled due to the content of \vr{string}.
...
\domain{By definition, all lists have finite length and are terminated by
the empty list.}
...
\domain{The \var{list} argument may be circular, but
...
Returns the name of \var{symbol} as a string that does not contain escapes.
It is an error
...
Implementations are free to add other names.
...
{\tt \#\backwhack{}x03BB}&; $\lambda$ (if name is supported)\\
...
immediately following \hyper{character} must be a delimiter character such as a
...
These procedures return \schtrue{} if the Unicode scalar values corresponding to their arguments are (respectively):
equal, monotonically increasing, monotonically decreasing,
monotonically nondecreasing, or monotonically nonincreasing.
...
is the same as the Unicode scalar value ordering.  This is true 
...
exclusive.  Thus if \var{start} and \var{end} are the same, a null
...
ASCII characters must not be forbidden.
...
\domain{It is an error if \var{start} and \var{end} 
are not exact integers satisfying the inequality
...
\proto{string->list}{ string start}{procedure}
\proto{string->list}{ string start end}{procedure}
...
characters of \var{string} between \var{start} (inclusive) and \var{end} (exclusive).
{\cf list\coerce{}string}
returns a newly allocated string formed from the elements in the list
\var{list}.
In both procedures, order is preserved.
If \var{start} is omitted, it is assumed to be 0.
If \var{end} is omitted, it is assumed to be the length of \var{string}.
...
\proto{string-copy}{ string start}{procedure}
\proto{string-copy}{ string start end}{procedure}}
...
\proto{string-fill!}{ string fill start}{procedure}
\proto{string-fill!}{ string fill start end}{procedure}}

\domain{It is an error if \var{fill} is not a character or is forbidden in strings,
or if \var{start} is greater than \var{end}.}
...
\proto{string-copy!}{ to at from start}{procedure}
\proto{string-copy!}{ to at from start end}{procedure}}

\domain{It is an error if \var{start} is greater than \var{end},
or \var{start} is less than zero, or \var{end} is greater than
the length of \var{from},
or \var{at} is less than zero or greater than the length of \var{to}.
It is also an error if the inequality
\texttt{({\cf >=} ({\cf -} ({\cf string-length} \var{to}) \var{at}) ({\cf -} \var{end} \var{start}))}
is false.}
Copies the characters of string \var{from} between \var{start} and \var{end}
to bytevector \var{to}, starting at \var{at}.  The order in which characters are
...
such circumstances.
If \var{start} is omitted, it is assumed to be 0.
If \var{end} is omitted, it is assumed to be the length of \var{from}.

The value returned by {\cf string-copy!} is unspecified.
...
\proto{vector->list}{ vector start}{procedure}
\proto{vector->list}{ vector start end}{procedure}
\proto{list->vector}{ list}{procedure}}

The {\cf vector->list} procedure returns a newly allocated list of the objects contained
in the elements of \var{vector} between \var{start} (inclusive) and \var{end} (exclusive).
The {\cf list->vector} procedure returns a newly
created vector initialized to the elements of the list \var{list}.

In both procedures, order is preserved.
If \var{start} is omitted, it is assumed to be 0.
If \var{end} is omitted, it is assumed to be the length of \var{vector}.
...
\proto{vector->string}{ vector start}{procedure}
\proto{vector->string}{ vector start end}{procedure}
\proto{string->vector}{ string}{procedure}
\proto{string->vector}{ string start}{procedure}
\proto{string->vector}{ string start end}{procedure}}

\domain{It is an error if any element is not a character allowed in strings.}
...
In both procedures, order is preserved.
If \var{start} is omitted, it is assumed to be 0.
If \var{end} is omitted, it is assumed to be the length of the first argument.
...
\domain{It is an error if \var{end} is greater than the length of \var{vector}.}
...
\proto{vector-fill!}{ vector fill start}{procedure}
...
\proto{vector-copy!}{ to at from start}{procedure}
\proto{vector-copy!}{ to at from start end}{procedure}}
...
or \var{at} is less than zero or greater than the length of \var{to}.
It is also an error if the inequality
\texttt{({\cf >=} ({\cf -} ({\cf vector-length} \var{to}) \var{at}) ({\cf -} \var{end} \var{start}))}
is false.}
Copies the elements of vector \var{from} between \var{start} (inclusive) and \var{end} (exclusive)
...
The value returned by {\cf vector-copy!} is unspecified.
...
\domain{It is an error if \var{start} is greater than \var{end},
or \var{start} is less than zero, or \var{end} is greater than
the length of \var{bytevector}.}
Returns a newly allocated bytevector containing the bytes in \var{bytevector}
between \var{start} (inclusive) and \var{end} (exclusive).
If \var{start} is omitted, it is assumed to be 0.
If \var{end} is omitted, it is assumed to be the length of \var{bytevector}.
...
\proto{bytevector-copy!}{ to at from start}{procedure}
\proto{bytevector-copy!}{ to at from start end}{procedure}}
...
is false.}
Copies the bytes of bytevector \var{from} between \var{start} (inclusive) and \var{end} (exclusive)
...
\proto{utf8->string}{ bytevector start} {procedure}
\proto{utf8->string}{ bytevector start end} {procedure}
\proto{string->utf8}{ string} {procedure}
\proto{string->utf8}{ string start} {procedure}
\proto{string->utf8}{ string start end} {procedure}}

\domain{It is an error to pass invalid byte sequences
...
a bytevector between \var{start} (inclusive) and \var{end} (exclusive)
and returns the corresponding string;
the {\cf string\coerce{}utf8} procedure encodes the characters of a
string between \var{start} (inclusive) and \var{end} (exclusive)
and returns the corresponding bytevector.
If \var{start} is omitted, it is assumed to be 0.
If \var{end} is omitted, it is assumed to be the length of
\var{bytevector} or \var{string}, respectively.
...
The \var{list}s may be circular, but if they are all circular, 
\var{proc} must make a non-local exit to avoid an infinite loop.
...
%% \begin{note}
%% The {\cf call/cc} procedure is capable of capturing continuations originating
...
\begin{note}
The interactions of \callcc{} with {\cf parameterize} and exception
handlers are defined by the formal semantics, but are hard to understand
in complex cases.  In addition, in some implementations \callcc{} consumes
an amount of memory that depends on the number of active continuations.
\end{note}
\end{entry}
 ...
 (lambda (k)
   (with-exception-handler
    ...
    (lambda ()
     (+ 1 (raise 'an-error))))))
         \ev \unspecified
 \>{\em and prints}  something went wrong%
\end{scheme}
...
\todo{consider removing unspecified effect}
These procedures call \var{proc} with one argument: the
...
an error that satisfies {\cf file-error?} is signalled if
the file named by \var{string} does not already exist; for
{\cf call-with-output-file},
the effect is unspecified if the file
already exists.
If the
file cannot be opened, an error that satisfies {\cf file-error?} is signalled.  If \var{proc} returns,
...
\domain{For {\cf with-input-from-file},
an error that satisfies {\cf file-error?} is signalled if the file named by \var{string} does not already exist; for
...
file.  If the file cannot be opened, an error that satisfies {\cf file-error?} is signalled.
...
an error that satisfies {\cf file-error?} is signalled.
\todo{consider removing unspecified effect}
...
an error that satisfies {\cf read-error?} is signalled.
...
\begin{entry}{%
\proto{read-string}{ length}{procedure}
\rproto{read-string}{ length port}{procedure}}

Reads the next \var{length} characters, or as many as are available before the end of file,
from the textual
...
However, {\cf display} must always terminate.  Thus if the
...
If {port} is omitted, it defaults to the
value returned by {\cf current-output-port}.
...
It is an error if \var{filename} is not a string. The {\cf file-exists?} procedure returns
...
It is an error if \var{filename} is not a string. The {\cf delete-file} procedure deletes the
...
that satisfies {\cf file-error?} is signalled.
...
an appropriate exit value for the operating system, if possible, and
