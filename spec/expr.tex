...
and \hyper{body} should be a sequence of zero or more generalized definitions
...
the location to which \hyper{variable} is bound.  If \hyper{variable} is not
bound either in some region\index{region} enclosing the {\cf set!}\ expression
or at top level, a Scheme implementation may act as if it were bound at top level.
...
sequence of zero or more generalized definitions followed by a
...
zero or more generalized definitions followed by
...
((\hyperi{variable} \hyperi{init}) \dotsfoo)\rm,%
\end{scheme}
and \hyper{body}\index{body} should be a sequence of
zero or more generalized definitions followed by
one or more expressions as described in section~\ref{lambda}. It is an error for a \hyper{variable} to appear more
than once in the list of variables being bound.
...
zero or more generalized definitions followed by a sequence of one or
...
generalized definitions followed by one or more expressions as described in section~\ref{lambda}.  In each \hyper{formals},
...
\begin{entry}{%
\proto{begin}{ \hyper{expr-or-def} \dotsfoo}{\exprtype}}
...
It causes the contained expressions and generalized definitions to be evaluated
...
which need to generate multiple generalized definitions and
...
{\cf (delay (force \var{expression}))}
might not.  Thus
iterative lazy algorithms that may result in a long series of chains of
...
\hyper{body} should be a sequence of one or more generalized definitions followed
...
An identifier appearing within a \hyper{pattern} may be a low line
...
Low lines also match arbitrary input elements but are not pattern variables
and so cannot be used to refer to those elements.  If a low line appears
in the \hyper{literal}s list, then that takes precedence and
low lines in the \hyper{pattern} match as literals.
Multiple low lines may appear in a \hyper{pattern}.
