...
\hyper{N} must be an unsigned decimal integer.  The syntax
...
A \hyper{transformser spec} has one of the following form:

\begin{entry}{%
\pproto{(syntax-rules (\hyper{literal} \dotsfoo) \hyper{syntax rule} \dotsfoo)}{\exprtype}
\pproto{(syntax-rules \hyper{ellipsis} (\hyper{literal} \dotsfoo) \hyper{syntax rule} \dotsfoo)}{\exprtype}}
...
(define-syntax simple-let
  (syntax-rules ()
    ((simple-let (head ... ((x . y) val) . tail) body1 body2 ...)
     (syntax-error ``name should be an identifier, but got'' (x . y)))
