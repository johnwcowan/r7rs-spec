...
with the {\cf delay}, {\cf lazy}, and {\cf parameterize} expression types.
...
called, the arguments will be stored in newly allocated locations
that are bound to the corresponding variables.

\item \hyper{variable}:
The procedure takes any number of arguments; when the procedure is
called, the sequence of actual arguments is converted into a newly
allocated list, and the list is stored in a newly allocated location
that is bound to
\hyper{variable}.
...
This form of {\cf begin} may appear as part of a \hyper{body}, or at the
\hyper{top-level}, or directly nested in a {\cf begin} that is itself of this form.
...
\hyper{body} should be a sequence of one or more definitions followed by expressions.  It is an error
...
A element in the input matches a literal identifier if and only if it is an
...
\item $P$ is of the form
      ...
      where $F$ is
      a proper list of $n$ elements, the first $e-1$ of which match
...
\item $P$ is of the form
      ...
      where $F$ is
      an list or improper list of $n$ elements, the first $e-1$ of which match
      $P_1$ through $P_{e-1}$,
      whose next $m-k$ elements each match $P_e$,
...
\item $P$ is of the form
      ...
      elements the first $e-1$ of which match $P_1$ through $P_{e-1}$,
      whose next $m-k$ elements each match $P_e$,
      and whose remaining $n-m$ elements matche $P_{m+1}$ through $P_n$; or
