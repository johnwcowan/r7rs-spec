...
The procedures {\cf force}, {\cf eager}, and {\cf make-parameter}
are described in this chapter because they are intimately associated
with the {\cf delay}, {\cf lazy}, and {\cf parameterized} expression types.
...
Numerical constants, string constants, character constants, bytevector
constants, and boolean constants evaluate to themselves; they need
not be quoted.
...
()}, is a legitimate expression.  In Scheme, it is an error for a combination
to have no subexpressions, so {\tt ()} is not a syntactically valid
expression.
\end{note}
...
It is an error unless all the \hyper{datum}s are distinct.
...
\ide{=>} alternate form, then the \hyper{expression} is evaluated.
It is an error unless its value is a procedure that accepts one argument.  This procedure is then
...
The following example prints {\cf 12}:
\begin{scheme}
(when (= 1 1.0)
  (display "1")
...
One restriction on {\cf letrec} is very important: it is an error if it is not possible
...
It is an error if it is not possible to evaluate each \hyper{init} without assigning or
...
where each \hyper{init} is an expression, and \hyper{body} should be a
sequence of zero or more definitions followed by a sequence of one or
more expressions.  It is an error for a variable to appear more than
once in the set of \hyper{formals}.

\semantics
The \hyper{init}s are evaluated in the current environment (in some
unspecified order) as if by invoking {\cf call-with-values}, the
variables occuring in the \hyper{formals} are bound to fresh locations
...
((\hyper{formals} \hyper{init}) \dotsfoo)\rm,%
\end{scheme}
and \hyper{body} should be a sequence of sequence of zero or more
definitions followed by one or more expressions.  In each \hyper{formals},
it is an error if any variable appears more than once.

\semantics
{\cf let-values*} is similar to {\cf let-values}, but the
...
including the bindings to its right as well as \hyper{body}.  Thus the
...
In all six constructs, it is an error if the continuation of each expression
used to compute initial values is invoked more than once.
...
is one of these three types.  In this case, it is an error if {\cf begin} form does not have
...
It may also appear as an ordinary expression.  In this case, it is an error if it does not have the shape
...
\begin{entry}{%
\proto{lazy}{ \hyper{expression}}{lazy library syntax}}

\todo{Fix.}

The {\cf lazy} construct
is similar to {\cf delay}, but it is an error for its argument not to evaluate to a promise.
...
Forces the value of a \var{promise} created by \ide{delay} or \ide{lazy},
section~\ref{delay}).\index{promise}  
If a value which is not a promise has already been computed, that value is
...
{\cf delay}, {\cf lazy}, and {\cf force} are mainly intended for
programs written in functional style.  The following examples are
not intended to illustrate good programming style, but they
...
{\cf lazy} are supported in some implementations:
...
\begin{entry}{%
\proto{eager} { \var{obj}}{lazy library procedure}}

{\cf Eager} returns a promise which when forced will return
...
\label{make-parameter}

Returns a newly allocated \defining{parameter object}\index{parameterize},
...
{\cf (converter \var{init})}, or of \var{init}
...
evaluated in a dynamic extent in which calls to the
...
{\cf raise-continuable} is re-invoked on the raised object within the dynamic
...
immediately by an at-sign (\atsign),\mainschindex{,@} then it is an error if the following
...
\begin{entry}{%
\proto{case-lambda}{ \hyperi{clause} \hyperii{clause} \dotsfoo}{case-lambda library syntax}}
...
It is an error if \hyper{n} is not an exact unsigned decimal integer datum.
...
it is an error for {\cf quasiquote} to contain them.
...
\syntax
It is an error unless each \hyper{literal}, as well as the \hyper{ellipsis} in the second form 
is an identifier and each
\hyper{syntax rule} is of the form
\begin{scheme}
