...
Thus, the dynamic extent of a call might not be a single, connected time
period.
...
In the final example, the dynamic extent of the cond-clauses is that of
the {\cf guard} which contains them, whereas the dynamic extent of the
re-raise is the same as the original raise.  This means that the first
raise will trigger the {\cf a} and {\cf b} setters, and then the {\cf c}
and {\cf d} setters will trigger. At this point, since the result is
\schfalse, the implementation should re-raise the object from the original
calling extent, thus triggering the {\cf a} and {\cf b} setters again,
before finally returning without re-entering again.
