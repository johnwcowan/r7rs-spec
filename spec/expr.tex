...
value(s) is(are) returned.  If \hyper{test} yields \schfalse{} and no
...
environment, and the value(s) of the last expression of \hyper{body}
is(are) returned.  Each binding of a \hyper{variable} has \hyper{body}
...
\begin{itemize}
\item It may appear as a form in a \hyper{body}, or at the
  \hyper{top-level}, or directly nested in a {\cf begin} form that
  appears in a \hyper{body}. In this case, the begin form must have
...
(let ((a 3)) 
  (cons ¡Ç(1 2)
        (cons a (cons 4 (cons ¡Çfive ¡Ç(6))))))
\end{scheme}

However, it is not equivalent to this expression:

\begin{scheme}
(let ((a 3)) (list (list 1 2) a 4 ¡Çfive 6))
\end{scheme}
...
unquote, or unquote-splicing appear in positions within a \hyper{qq
