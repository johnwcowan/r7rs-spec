...
It is an error if any the \hyper{datum}s are the same.
...
The following example outputs {\cf 12}:
\begin{scheme}
(when (= 1 1.0)
  (display "1")
...
where each \hyper{init} should be an expression, and \hyper{body} should be a
...
\syntax
It is an error if the \hyper{literal}s, as well as the \hyper{ellipsis} in the second form,
are not identifiers.
