...
(define-library (example grid)
  ...
  (import (scheme base))
  (begin
    ;; Create an NxM grid of zeros.
    (define (make n m)
      (let ((grid (make-vector n)))
        (do ((i 0 (+ i 1)))
            ((= i n) grid)
          (vector-set! grid i (make-vector m 0)))))
    (define (rows grid)
      (vector-length grid))
    (define (cols grid)
    ...
    (define (ref grid n m)
      (if (and (< -1 n (rows grid))
               (< -1 m (cols grid)))
          (vector-ref (vector-ref grid n) m)
          0))
    (define (put! grid n m v)
      (vector-set! (vector-ref grid n) m v))
    ;; Iterate over each element of the grid.
