...
\todo{Shinn: The sections below seem to cover this - do we want to
  summarize it here?}
%% If a top-level binding is redefined or modified, existing references
%% to the binding must reflect the change, however if an existing
%% non-syntax binding is redefined as syntax previous references may
%% remain unchanged.
...
\hyper{variable} is not bound, or is bound to a {\em syntax
  definition} (see below), then the definition will bind
...
\hyper{keyword} to the specified transformer, and existing references
to any non-syntax top-level binding for \hyper{keyword} remain unchanged.
