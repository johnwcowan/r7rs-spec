...
\item {\tt(rename \hyperi{import set} (\hyperi{$identifier$} \hyperii{$identifier$}) \dotsfoo)}
\end{itemize}
...
Like variable definitions, syntax definitions may appear at the outermost level or
...
Like other definitions, they may appear either at the outermost level or in a \ide{body}.
...
library declarations rather than expressions.
...
keyword imported from another library {\cf (bar)} is needed to expand the
library, then the syntax definition of the keyword must be imported from
{\cf (bar)} before the expansion of {\cf (foo)}.
