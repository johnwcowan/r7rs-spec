...
syntax definitions, and imports.  A collection of these same program
parts may also be encapsulated in a module to be reused by multiple
programs.
Expressions are described in chapter~\ref{expressionchapter};
definitions, syntax definitions, imports, and modules are the subject of the
...
namespace.  This section defines the notation for modules and their
semantics.
...
(module \hyper{module name}
  \hyper{module declaration} ...)
\end{scheme}

\hyper{module name} is a list of identifiers or exact integers used to
uniquely identify the module when importing from other programs or
modules.
...
\item{\tt(include \hyper{filename} ...)}

\item{\tt(include-ci \hyper{filename} ...)}

\item{\tt(cond-expand \hyper{cond-expand clause} ...)}

\end{itemize}

An \hyper{export} declaration specifies a list of identifiers which
...
\hyper{$identifier_1$} in each \tt{(\hyper{$identifier_1$}
  \hyper{$identifier_2$})} pairing, using \hyper{$identifier_2$} as the
external name.

An \hyper{import} declaration provides a way to import the identifiers
...
\item \tt{(only \hyper{import set} \hyper{identifier} ...)}
\item \tt{(except \hyper{import set} \hyper{identifier} ...)}
\item \tt{(prefix \hyper{import set} \hyper{identifier})}
\item \tt{(rename \hyper{import set} (\hyper{$identifier_1$} \hyper{$identifier_2$}) ...)}
\end{itemize}
...
exported with a \hyper{rename} form).  The additional \hyper{import
  set} forms modify this set as follows:

\begin{itemize}

\item An \hyper{only} form produces a subset of the given
...
\item An \hyper{except} form produces a subset of the given
...
\item A \hyper{rename} form modifies the given \hyper{import set},
...
\item A \hyper{prefix} form automatically renames all identifiers in
  the given \hyper{import set}, prefixing each with the specified
  \hyper{identifier}.

\end{itemize}

\hyper{import} declarations may also be made at the top-level of a
program.  In a module declaration, the same identifier may not be
imported more than once.

The \hyper{begin}, \hyper{include}, and \hyper{include-ci}
declarations are used to specify the commands and definitions that
make up the body of the module.  \hyper{begin} takes a list of forms
to be spliced literally, analagous to the top-level \hyper{begin}.
\hyper{include} and \hyper{include-ci} both take a filename, read all
top-level forms from the file and splices the results into the module
body.  \hyper{include-ci} uses a case-insensitive reader when reading
the forms from the file.

The forms from all \hyper{begin}, \hyper{include}, and
\hyper{include-ci} declarations are expanded in the order in which
they occur in the module declaration.

The \hyper{cond-expand} module declaration provides a way to
statically select expand different module declarations depending on
the platform or implementation under which the module is being
resolved.  A \hyper{cond-expand clause} must be of the following form:

\tt{(\hyper{feature requirement} \hyper{module declaration} ...)}

The last clause may be an ``else clause,'' which has the form

\tt{(else \hyper{module declaration} ...)}
...
\item \tt{\hyper{module name}}
\item \tt{(and \hyper{feature requirement} ...)}
\item \tt{(or \hyper{feature requirement} ...)}
\item \tt{(not \hyper{feature requirement})}
\end{itemize}

Each implementation maintains a list of feature identifier which are
...
each \hyper{feature identifier} and \hyper{module name} on the
implementation's list with \schtrue, and all other feature identifiers
and module names are replaced by \schfalse, then evaluating the
resulting expression as a Scheme boolean expression under the normal
interpretation of \hyper{and}, \hyper{or} and \hyper{not}.

A \hyper{cond-expand} form is then expanded by evaluating the
...
if there is an \hyper{else} clause its \hyper{module declaration}s are
included, otherwise the \hyper{cond-expand} has no effect.

\subsection{Standard Feature Identifiers}
...
exact-closed & All rational operations on exact values produce exact values. \\ \hline
ratios & / with exact arguments produces an exact result. \\ \hline
...
unix, darwin, linux, bsd, netbsd, freebsd, solaris, ... & Operating system flags (more than one may apply). \\ \hline
i386, x86\_64, ppc, sparc, sparc64, jvm, clr, llvm, ... & CPU architecture flags. \\ \hline
