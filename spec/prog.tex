...
section defines the notation and semantics for modules.
...
\hyper{module name} is a list whose members are identifiers or unsigned exact integers that is used to
identify the module uniquely when importing from other programs or
modules.
Modules whose first identifier is {\cf scheme} are reserved for use by this
report and future versions of this report.
Modules whose first identifier is {\cf srfi} are reserved for modules
...
read all top-level
forms from the files, and include the results into the module body or program as
though wrapped in a \ide{begin}.
...
The \ide{cond-expand} module declaration,
