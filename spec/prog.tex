...
record type definitions, {\cf import}s, {\cf cond-expand}s, and {\cf include}s.  A collection of program
...
at the top level of the programs
...
Implementations may provide a facility called a
\defining{REPL} (Read-Eval-Print Loop), where Scheme program parts can be
entered and evaluated one at a time.
A REPL may be interactive in nature, or may take its input from a file
or similar source or send its output to a file or similar destination or both.
For convenience and ease of use,
the ``top-level'' Scheme environment in a REPL
must not be empty, but must start out with 
at least the identifiers of the base library bound.
Other libraries may be imported or identifiers bound that are not
part of any library.
...
\item {\tt(rename \hyper{import set} (\hyperi{identifier} \hyperii{identifier}) \dotsfoo)}
...
In the first form, all of the identifiers in the named library's {\cf export}
...
\item \ide{rename} modifies the given \hyper{import set},
  ...
  identifiers is not found in the original set.
...
\item \ide{only} produces a subset of the given
  ...
  renaming).  It is an error if any of the listed identifiers is
...
\item \ide{except} produces a subset of the given
  ...
  renaming). It is an error if any of the listed identifiers is not
  found in the original set.

\end{itemize}

The top level of a program may also include \ide{import} declarations.

In a library declaration and at the top level, it is an error to import the same
...
the library.
The \ide{begin} declaration takes a list of expressions and definitions
analogous to the top-level \ide{begin}.  
Both the \ide{include} and
\ide{include-ci} declarations take one or more filenames expressed as string literals,
...
\ide{include} except that the resulting forms are spliced into the
current library form.  This can be used, for example, to share the
...
declarations from all \ide{begin}, \ide{include} and \ide{include-ci}
...
When a library is loaded, its outermost expressions are executed
in textual order.
...
program or library that imports bindings from a library will be from a
