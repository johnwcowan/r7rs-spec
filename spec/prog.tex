...
must not be empty, but must start out with variables
...
in textual order.  A library is loaded zero or more
times when it is imported by a program or by another library which is
about to be loaded, but must be loaded at least once in every program in
