...
An identifier\index{identifier} names either a type of syntax or
...
(These locations need not correspond to a full machine word.) A new value can be
...
is either mutable\index{mutable} or
immutable\index{immutable}.  Specifically: literal constants, the strings
returned by \ide{symbol->string}, and environments
returned by \ide{environment} and possibly by {\cf scheme-report-environment},
...
while not requiring other systems to actually distinguish between
...
\item Note that \hyper{cond clause}s appear in {\cf guard} expressions
