...
\item The {\cf write} procedure is now required to produce datum labels on
circular structure.
...
There is an \r5rs\ compatibility library.
...
\item The \rfivers\ procedure {\cf write} has been renamed {\cf write-simple}.
The new procedure {\cf write-shared} uses datum labels to report all
...
\item The behavior of {\cf eq?} and {\cf eqv?} on procedures is now unspecified.

\item The behavior of {\cf eqv?} on inexact numbers has been modified
and clarified.
...
neither been shown to be equivalent in power nor capable of implementing each
