...
\vest Identifiers that do not begin with a vertical bar, dot, numbers, characters, and booleans are
terminated by a \meta{delimiter} or by the end of the input.
Identifiers that begin with a vertical bar are terminated by another vertical bar.
...
and other languages). It is an error to use a non-Unicode character
...
\meta{whitespace} \: \meta{intraline whitespace} \| \meta{newline}
\> \| \meta{return}
...
Note that +i, -i and \meta{infinity} below are exceptions to the
